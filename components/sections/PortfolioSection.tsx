...
const techColors: Record<string, string> = {
  ...
  'Tailwind CSS': 'text-accent border-accent/30',
  'Framer Motion': 'text-neutral-300 border-neutral-500/30',
}
                ...
                <Card className="h-full p-6 flex flex-col">
                  ...
                  <div className="flex justify-between items-start mb-4">
                    ...
                    >
                      {t(
                        `portfolio.status.${
                          project.status === 'complete' ? 'complete' : 'inProgress'
                        }`
                      )}
                    </span>
                    {project.featured && (
                      <span className="text-xs text-neutral-500" aria-label="Featured project">
                        ★ Featured
                      </span>
                    )}
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2 mb-6">
                    {project.tech.map((tech) => (
                      <span
                        key={tech}
                        className={`text-xs px-2 py-0.5 rounded border font-mono ${getTechStyle(tech)}`}
                      >
                        {tech}
                      </span>
                    ))}
